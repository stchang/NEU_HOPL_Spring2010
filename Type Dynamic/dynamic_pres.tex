...
What types do we give the input and output of f? Remember, we need to be able to match all functions, so something like this: \code{\Lambda\tau.\lambda x:\tau\times\tau.\ldots}, and \code{\Lambda\tau.\lambda x:\tau\rightarrow\tau.\ldots}
...
%
%The contribution of the paper is that it demonstrates fine-grained interoperability between Scheme and Java. The paper uses \Dynamic to add dynamically typed expressions to Java to make it more compatible with Scheme. In Java, method invocation is always tied to the static type of an object. Unlike languages like Smalltalk or Python, you can't call a method on an object unless it was statically known. The authors add a \dynamic type and objects with this type are not inspected until runtime to see if they implement a method that is called.
%
